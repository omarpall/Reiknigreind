...
def montecarlo(board, player, n):
    ...
    va = np.zeros(na)
    for i in range(0, na):
        testboard = np.copy(board)
        testboard[moves[i]] = player
        movevalue = 0
        for j in range(0, n):
           
            tempboard = np.copy(testboard)
            tempmoves = legal_moves(tempboard)
            while(len(tempmoves) > 0):

                player = getotherplayer(player)
                randomMove = np.random.choice(tempmoves, 1)
                tempboard[randomMove] = player
                tempmoves = legal_moves(tempboard)
                if (1 == iswin(tempboard, player)):
                    if(player == 2):
                        movevalue += 1.0
                        print("win")
                    else:
                ...
                if(len(tempmoves == 0)):
                    ...
                    break
        va[i] = movevalue/n
        print("i",i,"va",va[i], "mv",movevalue)
    return moves[np.argmax(va)]
    ...
    for games in range(0, numgames):
        ...
        for move in range(0, 9):
            ...
            if(player == 1):
                ...
                action = montecarlo(np.copy(board), player, 10)
...
learnit(1000, epsilon, alpha, False)
